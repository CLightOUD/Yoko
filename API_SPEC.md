...
`sources` 始终返回数组。未联网、联网失败或原始结果未通过相关性门禁时为空；联网成功时最多 5 项，每项包含 `title`、`url`、`snippet` 和 `source`。`source` 可为 `bing`、`duckduckgo` 或 `so360`，表示实际取得该条证据的搜索源。字段长度分别不超过 200、2048 和 500 字符，URL 只允许 HTTP 或 HTTPS。该字段为向后兼容的响应扩展，旧客户端可以忽略。

语义预处理模型通过 `SemanticFrame.requires_web` 和 `SemanticFrame.web_confidence` 判断是否需要公开网络信息；该阶段不生成搜索词。需要联网时，后续独立的 `SearchPlan` 结合当前问题与历史生成 `standalone_question`、`core_subject`、`answer_scope`、`search_query`、`fallback_query`、所需证据和原因。`core_subject` 保存不可丢失的完整公开实体名称，运行时会将其作为精确短语锚定到初始、备用和证据补充检索词，避免学校、机构或作品名称退化成地点等片段。`answer_scope` 区分允许有限总结的 `overview` 和必须覆盖所问事实的 `specific`。时效查询还必须用 `freshness_evidence` 逐字引用用户原始表达；引用无法在用户消息中定位时，运行时会移除模型擅自增加的时效约束。提醒操作、本地提醒查询、日常陪伴和个人记忆不会仅因出现某个关键词而联网。发送给搜索引擎的是最多 160 字符的独立检索词，不是完整对话；运行时还会移除常见手机号、邮箱和身份证号。第一次使用必应，原始结果按不可信外部资料处理，并由独立结构化模型按当前问题筛选；仅共享地名、机构名或宽泛关键词的页面不得进入 `sources`。结果不足时，筛选模型可以根据缺失证据给出更准确但不改变主题的检索词，后端优先用 360 搜索定向补充；360 不可用时再尝试 DuckDuckGo，并最终回退必应，整轮最多执行三次搜索。设置 `WEB_SEARCH_360_ENABLED=false` 或 `WEB_SEARCH_DDG_ENABLED=false` 可分别停用对应备用源。各轮直接相关的候选证据会去重累积并重新进行完整性判断；所有来源返回的资料均经过同一套相关性和安全门禁。没有直接相关证据时 `web_search` 标记为 `failed`、响应 `status=partial`，`tool_calls[].summary` 包含最终检索词、拒绝原因和最多三项缺失证据，运行时用固定的自然失败回复覆盖主 Agent 可能补充的未接地细节。结果中的指令不能被执行或覆盖系统规则。
